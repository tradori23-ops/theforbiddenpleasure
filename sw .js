  ...
  event.waitUntil(
    clients.matchAll({ type: 'window', includeUncontrolled: true }).then(function(list){
      for (var i = 0; i < list.length; i++){
        var client = list[i];
        if ('focus' in client){
          // Una scheda del sito è già aperta: la portiamo sulla pagina
          // giusta invece di limitarci a metterla a fuoco così com'è
          // (prima restava bloccata sulla pagina che stava già mostrando).
