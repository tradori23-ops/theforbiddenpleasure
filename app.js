...
function initChatPage(){
  ...
  if(!otherUserId){ document.getElementById('chatNotFound').classList.remove('hidden'); return; }
    ...
    .then(function(threadId){
      ...
      fetch(SUPABASE_URL + '/rest/v1/profiles?id=eq.' + encodeURIComponent(otherUserId) + '&select=avatar_url,last_seen', { headers: communityHeaders() })
        ...
        .then(function(rows){
          var p = rows[0];
          if(!p) return;
          if(p.avatar_url) document.getElementById('chatOtherAvatar').src = p.avatar_url;
