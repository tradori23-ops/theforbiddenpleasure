...
var STR = {
  it:{
    ...
    "characters.eyebrow":"Dossier","characters.title":"I Cinque Sigilli","characters.desc":"Ogni personaggio guida una collana propria, con tono e mitologia distinti.","dossier.comingSoon":"I primi capitoli sono in arrivo.",
  ...
  en:{
    ...
    "characters.eyebrow":"Dossiers","characters.title":"The Five Seals","characters.desc":"Each character leads their own line, with a distinct tone and mythology.","dossier.comingSoon":"The first chapters are on their way.",
  ...
  es:{
    ...
    "characters.eyebrow":"Expedientes","characters.title":"Los Cinco Sellos","characters.desc":"Cada personaje lidera su propia colección, con tono y mitología distintos.","dossier.comingSoon":"Los primeros capítulos están en camino.",
  ...
  fr:{
    ...
    "characters.eyebrow":"Dossiers","characters.title":"Les Cinq Sceaux","characters.desc":"Chaque personnage mène sa propre collection, avec un ton et une mythologie distincts.","dossier.comingSoon":"Les premiers chapitres arrivent bientôt.",
  ...
  de:{
    ...
    "characters.eyebrow":"Dossiers","characters.title":"Die Fünf Siegel","characters.desc":"Jede Figur leitet eine eigene Reihe mit eigenem Ton und eigener Mythologie.","dossier.comingSoon":"Die ersten Kapitel erscheinen in Kürze.",
...
var CHAR_META = {
  ...
  Nox:{role:{it:"Il Fondatore",en:"The Founder",es:"El Fundador",fr:"Le Fondateur",de:"Der Gründer"},
    /* TESTO PLACEHOLDER — sostituisci con la bio reale di Nox appena pronta */
    bio:{it:"La firma dietro ogni collana della casa: la sua voce esce ora anche in prima persona.",
         en:"The signature behind every line in the house: his own voice now steps onto the page.",
         es:"La firma detrás de cada colección de la casa: ahora su propia voz sale a la página.",
         fr:"La signature derrière chaque collection de la maison : sa propre voix prend enfin la parole.",
         de:"Die Unterschrift hinter jeder Reihe des Hauses: jetzt tritt seine eigene Stimme auf die Seite."}}
};
...
function showEntryModeGate(){
  ...
  overlay.innerHTML =
    '<div style="max-width:400px;width:100%;background:rgba(26,17,19,0.92);border:1px solid rgba(201,162,77,0.35);border-radius:10px;padding:28px 24px;text-align:center;">' +
      ...
      '<div style="height:1px;background:rgba(201,162,77,0.25);margin:16px 0;"></div>' +
      '<button type="button" id="entryGateLuxtify" style="width:100%;background:transparent;border:1px solid #c9a24d;color:#c9a24d;border-radius:6px;padding:12px;font-family:\'Cinzel Decorative\',serif;letter-spacing:0.08em;font-size:14px;display:flex;align-items:center;justify-content:center;gap:8px;cursor:pointer;">💿 LUXTIFY</button>' +
      '<p style="color:#8a7a5e;font-size:11px;font-style:italic;margin:6px 0 0;">Solo musica, playlist e podcast</p>' +
  ...
  document.getElementById('entryGateLuxtify').addEventListener('click', function(){
    sessionStorage.setItem('lux_entry_mode', 'online'); // Luxtify richiede comunque la connessione
    window.location.href = 'luxtify.html';
  });
...
function refreshAuthUI(){
  ...
  if(isSignedIn()){
    ...
    if(isAdmin()){
      var founderUid = currentUserId();
      chip.innerHTML = acctChipAvatarHtml('', 'Nox Morningstar') + '<span>Nox Morningstar</span> ' + verifiedBadge('verified.founder');
      if(founderUid){
        fetch(SUPABASE_URL + '/rest/v1/profiles?id=eq.' + encodeURIComponent(founderUid) + '&select=avatar_url', { headers: communityHeaders() })
          .then(function(r){ return r.ok ? r.json() : []; })
          .then(function(rows){
            var avatarUrl = (rows[0] && rows[0].avatar_url) || '';
            chip.innerHTML = acctChipAvatarHtml(avatarUrl, 'Nox Morningstar') + '<span>Nox Morningstar</span> ' + verifiedBadge('verified.founder');
          })
          .catch(function(){}); // il nome+badge sono già a posto anche se questa chiamata fallisce
      }
    } else {
      var uid = currentUserId();
      var fallback = '@' + (currentUserEmail() || '').split('@')[0];
      chip.innerHTML = acctChipAvatarHtml('', fallback) + '<span>' + escapeHtml(fallback) + '</span>'; // placeholder finché arriva il profilo vero
      if(uid){
        fetch(SUPABASE_URL + '/rest/v1/profiles?id=eq.' + encodeURIComponent(uid) + '&select=display_name,avatar_url,verified', { headers: communityHeaders() })
          .then(function(r){ return r.ok ? r.json() : []; })
          .then(function(rows){
            var p = rows[0] || {};
            var name = (p.display_name && p.display_name !== t('notif.someone')) ? p.display_name : fallback;
            chip.innerHTML = acctChipAvatarHtml(p.avatar_url || '', name) + '<span>' + escapeHtml(name) + '</span> ' + verifiedBadge('verified.commenter', !!p.verified);
          })
          .catch(function(){}); // il fallback col solo nome resta visibile anche se questa chiamata fallisce
      }
    }
...
/* Riga "il tuo codice": due numeri affidabili, contati da dati che esistono
   già altrove nel sito — niente di nuovo lato database. "Posseduti" usa la
   stessa fonte della tab Acquisti (richieste con prefisso [ACQUISTO]); "in
   corso" riusa readingProgressMap, con lo stesso criterio (iniziato, non
   finito) già usato per la fascia di progresso sulla card. Niente "collana
   preferita": gli acquisti sono testo libero, non collegati al catalog_id,
   quindi non c'è modo affidabile di risalire al personaggio. */
function refreshMyComicsCodex(){
  var codexEl = document.getElementById('mycomicsCodex');
  if(!codexEl) return;
  if(!isSignedIn()){ codexEl.classList.add('hidden'); return; }

  var catItems = getCatalog();
  var inProgressCount = catItems.filter(function(i){
    var lp = readingProgressMap[i.id];
    var total = (i.pages && i.pages.length) || 0;
    return typeof lp === 'number' && total > 1 && lp > 0 && lp < total - 1;
  }).length;

  fetch(SUPABASE_URL + '/rest/v1/requests?user_id=eq.' + encodeURIComponent(currentUserId()) + '&select=body', {
    headers: communityHeaders()
  }).then(function(r){ return r.ok ? r.json() : []; }).then(function(rows){
    var ownedCount = (rows || []).filter(function(r){ return (r.body||'').indexOf('[ACQUISTO]') === 0; }).length;
    codexEl.innerHTML =
      '<span>' + t('mycomics.codex.owned').replace('{n}', '<b>'+ownedCount+'</b>') + '</span>' +
      '<span>' + t('mycomics.codex.inProgress').replace('{n}', '<b>'+inProgressCount+'</b>') + '</span>';
    codexEl.classList.remove('hidden');
  }).catch(function(e){ console.warn('mycomics codex failed:', e); codexEl.classList.add('hidden'); });
}
  ...
  return (
    '<div class="card-idx tome-card" data-character="'+escapeHtml(item.character||'')+'">'+
      ...
      '<div class="tome-price-row">'+
        ...
        '<button type="button" class="tome-details-toggle" data-toggle-details aria-label="'+t('card.details')+'" aria-expanded="false">'+
          '<span class="gate-frame"><span class="gate-arch"></span><span class="gate-door gate-door-l"></span><span class="gate-door gate-door-r"></span></span>'+
...
function wireTomeCard(card, item){
  ...
  detailsToggle.addEventListener('click', function(e){
    e.stopPropagation();
    var expanded = detailsBox.classList.toggle('open');
    detailsToggle.classList.toggle('open', expanded);
    detailsToggle.setAttribute('aria-expanded', expanded ? 'true' : 'false');
    detailsToggle.setAttribute('aria-label', expanded ? t('card.detailsLess') : t('card.details'));
    // il testo diventa misurabile solo ora che la sezione non è più a max-height:0
...
function renderPixaiCarousel(){
  var wrap = document.getElementById('pixaiCarousel');
  if(!wrap) return;
  if(!matureVisible){ wrap.classList.add('hidden'); return; }
  var items = getCatalog().filter(function(i){ return (i.genres||[]).indexOf('PixAI') !== -1; });
  if(items.length === 0){ wrap.classList.add('hidden'); return; }
  wrap.classList.remove('hidden');
  var row = wrap.querySelector('.pixai-carousel-row');
  row.innerHTML = '';
  items.forEach(function(item){
    row.insertAdjacentHTML('beforeend', buildTomeCardHtml(item));
  });
  var cardEls = row.querySelectorAll('.tome-card');
  items.forEach(function(item, i){ wireTomeCard(cardEls[i], item); });
  renderPixaiCommunity(items);
}

/* Community PixAI — in cima, un invito ben visibile al vero Server PixAI
   (se l'admin ne ha creato uno con quel nome); sotto, la fascia di chi ha
   messo PixAI nei preferiti — riusa il sistema di amicizia/messaggi già
   esistente, nessuna stanza nuova, nessun anonimato. */
function renderPixaiCommunity(pixaiItems){
  var box = document.getElementById('pixaiCommunity');
  var row = document.getElementById('pixaiCommunityRow');
  var countEl = document.getElementById('pixaiCommunityCount');
  var ctaBox = document.getElementById('pixaiServerCta');
  if(!box || !row) return;

  fetch(SUPABASE_URL + '/rest/v1/communities?name=ilike.pixai&select=id,name,icon&limit=1', { headers: communityHeaders() })
    .then(function(r){ return r.ok ? r.json() : []; })
    .then(function(rows){
      if(!ctaBox) return;
      if(rows[0]){
        ...
        ctaBox.innerHTML =
          '<span class="pixai-server-cta-icon">' + escapeHtml(s.icon || '🔮') + '</span>' +
          '<span class="pixai-server-cta-text">' + t('pixai.serverCta') + '</span>' +
          '<span class="pixai-server-cta-arrow">→</span>';
        ctaBox.onclick = function(){
          sessionStorage.setItem('lux_pending_community_tab', 'servers');
          sessionStorage.setItem('lux_pending_server_id', s.id);
    ...
    .catch(function(e){ console.warn('PixAI server lookup failed:', e); if(ctaBox) ctaBox.classList.add('hidden'); });

  if(!isSignedIn()){ box.classList.add('hidden'); return; }
  var ids = pixaiItems.map(function(i){ return i.id; });
  if(ids.length === 0){ box.classList.add('hidden'); return; }
  var uid = currentUserId();

  fetch(SUPABASE_URL + '/rest/v1/favorites?catalog_id=in.(' + ids.map(encodeURIComponent).join(',') + ')&select=user_id', { headers: communityHeaders() })
    .then(function(r){ return r.ok ? r.json() : []; })
    .then(function(favRows){
      var userIds = Array.from(new Set(favRows.map(function(f){ return f.user_id; }))).filter(function(id){ return id !== uid; });
      if(userIds.length === 0){ box.classList.add('hidden'); return; }
      return Promise.all([
        loadFriendshipMap(),
        fetch(SUPABASE_URL + '/rest/v1/profiles?id=in.(' + userIds.map(encodeURIComponent).join(',') + ')&select=id,display_name,avatar_url,verified,last_seen', { headers: communityHeaders() })
          .then(function(r){ return r.ok ? r.json() : []; })
        ...
        profiles.forEach(function(p){
          ...
          card.innerHTML =
            '<span class="user-directory-avatar-wrap" style="width:52px;height:52px;cursor:pointer;">'+avatarHtml+
              '<span class="user-directory-dot '+(online?'online':'offline')+'"></span></span>'+
            '<div class="pixai-community-name">'+escapeHtml(p.display_name || t('notif.someone'))+verifiedBadge('verified.commenter', !!p.verified)+'</div>';
          card.querySelector('.user-directory-avatar-wrap').addEventListener('click', function(){
            window.location.href = 'profile.html?user=' + encodeURIComponent(p.id);
          });
          card.appendChild(contattiFriendButton(p.id, p.display_name, false));
          row.appendChild(card);
        });
      });
    })
    .catch(function(e){ console.warn('PixAI community load failed:', e); box.classList.add('hidden'); });
}
...
function switchCommunityTab(tab){
  ...
  if(tab === 'servers') renderServerRail();
  if(tab === 'events') renderEventsTab();
}

/* ============ SERVER (creati da Amministra: canale reale + elenco iscritti) ============ */
var serverListCache = null;
var activeServerId = null;

function renderServerRail(){
  var rail = document.getElementById('serverRailRow');
  var empty = document.getElementById('serverEmptyState');
  if(!rail) return;
  fetch(SUPABASE_URL + '/rest/v1/communities?select=*&order=created_at.asc', { headers: communityHeaders() })
    .then(function(r){ return r.ok ? r.json() : []; })
    .then(function(servers){
      serverListCache = servers;
      var pendingServerId = sessionStorage.getItem('lux_pending_server_id');
      if(pendingServerId){
        sessionStorage.removeItem('lux_pending_server_id');
        if(servers.some(function(s){ return s.id === pendingServerId; })) activeServerId = pendingServerId;
      }
      if(servers.length === 0){
        rail.innerHTML = '';
        if(empty) empty.classList.remove('hidden');
        document.getElementById('serverDetailBox').innerHTML = '';
        return;
      }
      ...
      servers.forEach(function(s){
        var btn = document.createElement('button');
        btn.type = 'button';
        btn.className = 'server-rail-icon' + (s.id === activeServerId ? ' active' : '');
        btn.textContent = s.icon || '💬';
        btn.title = s.name;
        btn.addEventListener('click', function(){ selectServer(s.id); });
        rail.appendChild(btn);
      });
      selectServer(activeServerId && servers.some(function(s){ return s.id === activeServerId; }) ? activeServerId : servers[0].id);
    })
...
function selectServer(id){
  ...
  document.querySelectorAll('.server-rail-icon').forEach(function(btn, i){
    btn.classList.toggle('active', serverListCache[i] && serverListCache[i].id === id);
  });
...
function refreshServerJoinState(serverId, btn){
  if(!isSignedIn()){
    btn.textContent = t('servers.join');
    btn.classList.add('btn-primary');
    btn.onclick = function(){ openAuth('login'); };
    return;
  }
  var uid = currentUserId();
  fetch(SUPABASE_URL + '/rest/v1/community_members?community_id=eq.' + encodeURIComponent(serverId) + '&user_id=eq.' + encodeURIComponent(uid) + '&select=user_id', { headers: communityHeaders() })
    .then(function(r){ return r.ok ? r.json() : []; })
    .then(function(rows){
      var joined = rows.length > 0;
      btn.textContent = joined ? t('servers.joined') : t('servers.join');
      btn.classList.toggle('btn-primary', !joined);
      btn.classList.toggle('btn-ghost', joined);
      btn.onclick = function(){ toggleServerMembership(serverId, joined, btn); };
    });
}

function toggleServerMembership(serverId, currentlyJoined, btn){
  var session = getSession();
  if(!session) return;
  var uid = currentUserId();
  btn.disabled = true;
  var req = currentlyJoined
    ? fetch(SUPABASE_URL + '/rest/v1/community_members?community_id=eq.' + encodeURIComponent(serverId) + '&user_id=eq.' + encodeURIComponent(uid), {
        method:'DELETE', headers:{ 'apikey':SUPABASE_ANON_KEY, 'Authorization':'Bearer ' + session.access_token }
      })
    : fetch(SUPABASE_URL + '/rest/v1/community_members', {
        method:'POST',
        headers:{ 'apikey':SUPABASE_ANON_KEY, 'Authorization':'Bearer ' + session.access_token, 'Content-Type':'application/json' },
        body: JSON.stringify({ community_id: serverId, user_id: uid })
      });
  req.then(function(r){
    if(!r.ok) throw new Error('join/leave server fallito');
    btn.disabled = false;
    refreshServerJoinState(serverId, btn);
    renderServerMembers(serverId);
...
function renderServerMembers(serverId){
  ...
  fetchServerMemberProfiles(serverId)
    ...
    .catch(function(e){
      box.innerHTML = '<p class="form-note">' + t('community.loadError') + '</p>';
      console.warn('Server members load failed:', e);
    });
}

/* ============ EVENTI (fiere, cinema, incontri) — countdown + traccia post-evento ============
   Nessuna riga viene mai davvero cancellata: una volta passata event_date,
  ...
  fetch(SUPABASE_URL + '/rest/v1/community_events?select=*&order=event_date.asc', { headers: communityHeaders() })
    ...
    .then(function(events){
      ...
      if(upcoming.length > 2){
        ...
        more.className = 'server-channel-link';
        more.style.marginTop = '4px';
        more.textContent = t('events.seeAll');
        more.addEventListener('click', function(){ sessionStorage.setItem('lux_pending_community_tab', 'events'); });
        list.appendChild(more);
      }
      tickEventCountdowns();
      clearInterval(eventCountdownTimer);
      eventCountdownTimer = setInterval(tickEventCountdowns, 1000);
    })
    .catch(function(e){ console.warn('Home events load failed:', e); box.classList.add('hidden'); });
}

function renderEventsTab(){
  var upBox = document.getElementById('eventsUpcomingList');
  var pastBox = document.getElementById('eventsPastList');
  if(!upBox || !pastBox) return;
  upBox.innerHTML = '<p class="form-note">…</p>';
  pastBox.innerHTML = '';
  clearInterval(eventCountdownTimer);
  ...
  fetch(SUPABASE_URL + '/rest/v1/community_events?select=*&order=event_date.asc', { headers: communityHeaders() })
    ...
    .then(function(events){
      ...
      upcoming.forEach(function(ev){ upBox.appendChild(buildEventCard(ev)); });
      ...
      if(past.length === 0){ pastBox.innerHTML = '<p class="form-note">' + t('events.noPast') + '</p>'; return; }
      renderEventTraces(pastBox, past.slice(0, 20));
    })
    .catch(function(e){
      upBox.innerHTML = '<p class="form-note">' + t('community.loadError') + '</p>';
...
function buildEventCard(ev){
  ...
  var attendBtn = card.querySelector('[data-attend]');
...
function tickEventCountdowns(){
  ...
  document.querySelectorAll('.countdown[data-target]').forEach(function(box){
    ...
    box.querySelector('[data-cd="d"]').textContent = d;
...
function refreshEventAttendState(eventId, btn, countEl){
  fetch(SUPABASE_URL + '/rest/v1/event_attendees?event_id=eq.' + encodeURIComponent(eventId) + '&select=user_id', { headers: communityHeaders() })
    .then(function(r){ return r.ok ? r.json() : []; })
    .then(function(rows){
      countEl.textContent = rows.length + ' ' + t('events.attendeeCount');
      var going = isSignedIn() && rows.some(function(r){ return r.user_id === currentUserId(); });
      btn.textContent = going ? t('events.going') : t('events.attend');
      btn.classList.toggle('going', going);
    })
    .catch(function(){});
}

function toggleEventAttendance(eventId, btn, countEl){
  if(!isSignedIn()){ openAuth('login'); return; }
  var session = getSession();
  var uid = currentUserId();
  var wasGoing = btn.classList.contains('going');
  btn.disabled = true;
  var req = wasGoing
    ? fetch(SUPABASE_URL + '/rest/v1/event_attendees?event_id=eq.' + encodeURIComponent(eventId) + '&user_id=eq.' + encodeURIComponent(uid), {
        method:'DELETE', headers:{ 'apikey':SUPABASE_ANON_KEY, 'Authorization':'Bearer ' + session.access_token }
      })
    : fetch(SUPABASE_URL + '/rest/v1/event_attendees', {
        method:'POST',
        headers:{ 'apikey':SUPABASE_ANON_KEY, 'Authorization':'Bearer ' + session.access_token, 'Content-Type':'application/json' },
        body: JSON.stringify({ event_id: eventId, user_id: uid })
      });
  req.then(function(r){
    if(!r.ok) throw new Error('event attendance toggle failed');
    btn.disabled = false;
    refreshEventAttendState(eventId, btn, countEl);
  }).catch(function(e){ btn.disabled = false; console.warn('Event attendance toggle failed:', e); });
}
...
function loadDiaryFeed(containerId, forcedUserId){
  ...
  fetch(url, { headers: communityHeaders() })
    ...
    .catch(function(e){ feed.innerHTML = '<p class="form-note">' + t('community.loadError') + '</p>'; console.warn('Diary feed load failed:', e); });
}
...
function loadDiaryComments(postId, box){
  ...
  fetch(SUPABASE_URL + '/rest/v1/post_comments?post_id=eq.'+encodeURIComponent(postId)+'&select=*&order=created_at.asc', { headers: communityHeaders() })
    ...
    .catch(function(e){ box.innerHTML = '<p class="form-note">' + t('community.loadError') + '</p>'; console.warn('Post comments load failed:', e); });
}
...
function loadChannels(){
  if(!isSignedIn()) return;
  fetch(SUPABASE_URL + '/rest/v1/channels?community_id=is.null&select=*&order=created_at.desc', { headers: communityHeaders() })
...
function renderFriendContactList(friendsBox, accepted, uid){
  ...
  var friendshipByOtherId = {};
  accepted.forEach(function(f){
    var oid = f.requester_id === uid ? f.addressee_id : f.requester_id;
    friendshipByOtherId[oid] = f.id;
  });
  friendsBox.innerHTML = '<p class="form-note">…</p>';
  fetch(SUPABASE_URL + '/rest/v1/profiles?id=in.(' + otherIds.map(encodeURIComponent).join(',') + ')&select=id,display_name,avatar_url,last_seen', { headers: communityHeaders() })
    .then(function(r){ return r.ok ? r.json() : []; })
    .then(function(profiles){
      ...
      profiles.forEach(function(p){
        ...
        removeBtn.addEventListener('click', function(e){
          e.stopPropagation();
          removeFriend(friendshipByOtherId[p.id], name, row);
        });
...
function removeFriend(friendshipId, otherName, rowEl){
...
function toggleFollow(otherUserId, btnEl){
  ...
  req.then(function(r){
    ...
    btnEl.classList.toggle('active', nowFollowing);
...
function renderContattiScopri(query){
  ...
  Promise.all([loadFriendshipMap(), fetch(profilesUrl, { headers: communityHeaders() }).then(function(r){ return r.ok ? r.json() : []; })])
    .then(function(results){
      ...
      profiles.forEach(function(p){
        loadFollowerCount(p.id).then(function(n){
          var metaText = n + ' ' + t('pubProfile.statFollowers');
...
function fetchSongsForTitle(catalogId){
  return fetch(SUPABASE_URL + '/rest/v1/songs?catalog_id=eq.' + encodeURIComponent(catalogId) + '&select=*,catalog(title,cover_url,mature)&order=position.asc,created_at.asc', {
...
function fetchPlaylists(){
  // solo le playlist pubbliche/ufficiali (create da Admin) — quelle personali
  // degli utenti restano private e si vedono con fetchMyPlaylists()
  return fetch(SUPABASE_URL + '/rest/v1/playlists?is_public=eq.true&select=*&order=created_at.desc', {
    headers:{ 'apikey':SUPABASE_ANON_KEY }
  }).then(function(r){ return r.ok ? r.json() : []; }).catch(function(){ return []; });
}
function fetchMyPlaylists(){
  ...
  var session = getSession();
  return fetch(SUPABASE_URL + '/rest/v1/playlists?created_by=eq.' + encodeURIComponent(uid) + '&select=*&order=created_at.desc', {
    headers:{ 'apikey':SUPABASE_ANON_KEY, 'Authorization':'Bearer ' + session.access_token }
  }).then(function(r){ return r.ok ? r.json() : []; }).catch(function(){ return []; });
}
function fetchPlaylistSongs(playlistId){
  ...
  return fetch(SUPABASE_URL + '/rest/v1/playlist_songs?playlist_id=eq.' + encodeURIComponent(playlistId) + '&select=position,songs(*,catalog(mature))&order=position.asc', {
    headers: headers
  }).then(function(r){ return r.ok ? r.json() : []; }).catch(function(){ return []; })
...
function ensureMusicLibraryStyle(){
  ...
  styleEl.textContent = [
    ...
    '.ml-section-title{font-family:"Space Mono",monospace;font-size:10.5px;letter-spacing:0.14em;text-transform:uppercase;color:var(--ml-dim);margin-bottom:10px;}',
    '.ml-cards-auto{margin-bottom:0;}',
...
function renderMusicLibraryTab(tab){
  ...
  if(isAlbums){
    ...
    /* "Playlist automatiche": una per collana (Lucifer, Lilith...), generata da sola dai brani
       già presenti — non richiede nessuna playlist creata a mano. */
    Promise.all([fetchAllSongsWithAlbum(), fetchPlaylists()]).then(function(results){
      var songs = results[0], playlists = results[1];
      var byColl = {}, order = [];
      songs.forEach(function(row){
        var coll = (row.catalog && row.catalog.character) || null;
        if(!coll) return; // brani senza collana (caricati liberamente) non entrano in una playlist automatica
      ...
      var autoHtml = collOrder.length ? (
        ...
        collOrder.map(function(coll, i){
          var list = byColl[coll];
          var cover = list.filter(function(s){ return s.cover_url || (s.catalog && s.catalog.cover_url); })[0];
      ...
      var mineHtml = '<div class="ml-section-title" style="margin-top:' + (autoHtml ? '22px' : '0') + ';">Le mie playlist</div>' +
        (playlists.length === 0 ? '<p class="form-note">Nessuna playlist ancora.</p>' : playlists.map(function(p, i){
      ...
      grid.innerHTML = autoHtml + mineHtml;
      Array.prototype.forEach.call(grid.querySelectorAll('[data-auto-idx]'), function(el){
        el.addEventListener('click', function(){
          var coll = collOrder[Number(el.dataset.autoIdx)];
          closeMusicLibrary();
          openMusicPlayer(byColl[coll], 'Tutto ' + coll);
        });
      });
...
/* ============ LUXTIFY — home dedicata (luxtify.html) ============ */
function renderLuxtifyHome(){
  var section = document.getElementById('luxtifySection');
  if(!section) return; // non siamo su luxtify.html

  ensureMusicLibraryStyle(); // riusa lo stesso foglio di stile della libreria

  initLuxtifyAdult();
  renderLuxtifyContinueListening();
  ...
  Promise.all([fetchAllSongsWithAlbum(), fetchPlaylists(), fetchPodcasts()]).then(function(results){
    ...
    luxtifySongsCache = songs;
    renderLuxtifyAll();
    renderLuxtifyMixes(songs);
    renderLuxtifyTopArtists(songs);
    renderLuxtifyStats(songs, playlists, podcasts);
  });
      ...
      html += '<div class="luxtify-section"><div class="luxtify-section-title">Brani</div>' +
        songs.map(function(s, i){
          return '<div class="luxtify-inline-card" data-search-song="' + i + '" style="margin-bottom:8px;">' +
            ...
            '<div style="flex:1;min-width:0;"><div style="color:#f0e4cd;font-size:12px;font-weight:600;">' + escapeHtml(s.title) + lxExplicitTag(s) + '</div>' +
...
function renderLuxtifyStats(songs, playlists, podcasts){
  var el = document.getElementById('luxtifyStats');
  if(!el) return;
  var albumCount = {};
  songs.forEach(function(s){ if(s.catalog_id) albumCount[s.catalog_id] = true; });
  var albumTotal = Object.keys(albumCount).length;
  el.textContent = songs.length + (songs.length === 1 ? ' brano' : ' brani') +
    ' · ' + albumTotal + (albumTotal === 1 ? ' album' : ' album') +
    ' · ' + playlists.length + (playlists.length === 1 ? ' playlist' : ' playlist') +
    ' · ' + podcasts.length + (podcasts.length === 1 ? ' podcast' : ' podcast');
}

function renderLuxtifyContinueListening(){
  var box = document.getElementById('luxtifyContinue');
  if(!box) return;
  var raw;
  try { raw = localStorage.getItem('luxtifyLastPlayed'); } catch(e){ raw = null; }
  if(!raw){ box.closest('.luxtify-section').classList.add('hidden'); return; }
  var last;
  ...
  box.innerHTML =
    ...
    '</div>' +
    '<button type="button" id="luxtifyContinueBtn" style="width:28px;height:28px;border-radius:50%;background:#6e1423;border:none;display:flex;align-items:center;justify-content:center;flex-shrink:0;cursor:pointer;">▶</button>';
  document.getElementById('luxtifyContinueBtn').addEventListener('click', function(){
    fetch(SUPABASE_URL + '/rest/v1/songs?id=eq.' + encodeURIComponent(last.id) + '&select=*', { headers:{ 'apikey':SUPABASE_ANON_KEY } })
      .then(function(r){ return r.ok ? r.json() : []; }).then(function(rows){
        if(rows[0]) openMusicPlayer([rows[0]], last.contextLabel || '');
      });
  });
}

/* ============ LUXTIFY — layout "vetrina" e brani espliciti (18+) ============
   Un brano è esplicito se è segnato così (songs.explicit) oppure se il suo album è collegato a un fumetto 18+.
   Con l'interruttore 18+ spento i brani espliciti si vedono ma sono BLOCCATI: copertina sfocata, lucchetto, e al tocco
   compare la conferma dell'età (la stessa del resto del sito). Con l'interruttore acceso si ascoltano normalmente. */
var luxtifySongsCache = [];
var luxtifyTag = '';
var luxtifyCurrentList = [];
var luxtifyCurrentLabel = '';
var pendingMatureAction = null;
var LX_LOCK_HTML = '<div class="lx-lockov"><svg class="lx-ico" viewBox="0 0 24 24"><rect x="5" y="11" width="14" height="9" rx="2"/><path d="M8 11V8a4 4 0 0 1 8 0v3"/></svg>18+</div>';

function songIsExplicit(s){ return !!(s && (s.explicit === true || (s.catalog && s.catalog.mature === true))); }
function songIsLocked(s){ return songIsExplicit(s) && !matureVisible; }
function lxExplicitTag(s){
  return songIsExplicit(s) ? ' <span class="lx-tag" title="Contenuto esplicito">' + (matureVisible ? 'E' : '18+') + '</span>' : '';
}
function lxCover(url, locked){
  return '<div class="lx-cvwrap' + (locked ? ' lx-lock' : '') + '"><img class="lx-cov" src="' + escapeHtml(url || MUSIC_COVER_FALLBACK) + '" alt=""' + coverImgAttrs() + '>' + (locked ? LX_LOCK_HTML : '') + '</div>';
}
function lxSongCover(s){ return s.cover_url || (s.catalog && s.catalog.cover_url) || ''; }

/* generi: "Upbeat Italian Pop, Indie Pop" diventa due filtri distinti invece di un unico pulsante lunghissimo */
function lxSongTags(s){ return String(s.genre || '').split(',').map(function(x){ return x.trim(); }).filter(Boolean); }
function lxAllTags(songs){
  var seen = {}, out = [];
  songs.forEach(function(s){
    lxSongTags(s).forEach(function(tg){
      var k = tg.toLowerCase();
      if(!seen[k]){ seen[k] = true; out.push(tg); }
    });
  });
  return out;
}
function lxMatchesTag(s){
  if(!luxtifyTag) return true;
  var k = luxtifyTag.toLowerCase();
  return lxSongTags(s).some(function(x){ return x.toLowerCase() === k; });
}
...
function enableAdultMode(){
  ...
  var sw = document.getElementById('matureSwitch');
  if(sw && !sw.checked) sw.click(); else matureVisible = true;
}
function askLuxtifyAdult(afterConfirm){
  var verified = false;
  try { verified = localStorage.getItem('lux_age_ok') === '1'; } catch(e){}
  if(verified){ enableAdultMode(); if(afterConfirm) afterConfirm(); return; }
  pendingMatureAction = afterConfirm || null;
  openMatureModal();
}
function runPendingMatureAction(){
  var f = pendingMatureAction;
  pendingMatureAction = null;
  if(f) f();
}
function initLuxtifyAdult(){
  var btn = document.getElementById('luxtifyAdultToggle');
  ...
  // su questa pagina non c'è il menu principale, quindi i pulsanti della conferma età li collega Luxtify
  var enter = document.getElementById('gateEnter');
  var leave = document.getElementById('gateLeave');
  var modal = document.getElementById('matureModal');
  if(enter) enter.addEventListener('click', function(){
    try { localStorage.setItem('lux_age_ok', '1'); } catch(e){}
    closeMatureModal();
    setLuxtifyAdult(true);
    runPendingMatureAction();
  });
  if(leave) leave.addEventListener('click', function(){ closeMatureModal(); pendingMatureAction = null; });
  if(modal) modal.addEventListener('click', function(e){ if(e.target.id === 'matureModal'){ closeMatureModal(); pendingMatureAction = null; } });
}

/* ---- in evidenza (era "Consigliata per oggi") ---- */
function renderLuxtifyHero(songs){
  var box = document.getElementById('luxtifyDailyPick');
  if(!box) return;
  var pool = songs.filter(function(s){ return !songIsLocked(s); }); // in prima pagina si preferisce un brano che tutti possono ascoltare
  if(pool.length === 0) pool = songs;
  if(pool.length === 0){ box.classList.add('hidden'); return; }
  // scelta stabile per tutta la giornata (stesso giorno = stesso brano per tutti)
  var pick = pool[Math.floor(Date.now() / 86400000) % pool.length];
  var locked = songIsLocked(pick);
  box.classList.remove('hidden');
  box.innerHTML =
    '<img class="lx-hero-img' + (locked ? ' lx-blur' : '') + '" src="' + escapeHtml(lxSongCover(pick) || MUSIC_COVER_FALLBACK) + '" alt=""' + coverImgAttrs() + '>' +
    '<div class="lx-hero-shade"></div>' +
    '<div class="lx-hero-txt"><div style="min-width:0">' +
      '<div class="lx-hero-kicker">IN EVIDENZA · SCELTA DI OGGI' + lxExplicitTag(pick) + '</div>' +
      '<span class="lx-hero-title">' + escapeHtml(pick.title) + '</span>' +
      '<div class="lx-hero-artist">' + escapeHtml(pick.artist || '') + '</div>' +
    '</div><button type="button" class="lx-play" aria-label="Riproduci">▶</button></div>' + (locked ? LX_LOCK_HTML : '');
  box.onclick = function(){ openMusicPlayer([pick], (pick.catalog && pick.catalog.title) || ''); };
}

/* ---- filtri per genere ---- */
function renderLuxtifyGenreChips(songs){
  var box = document.getElementById('luxtifyGenreChips');
  if(!box) return;
  var tags = lxAllTags(songs);
  if(tags.length === 0){ box.classList.add('hidden'); return; }
  box.classList.remove('hidden');
  box.innerHTML = '<button type="button" class="luxtify-chip' + (luxtifyTag ? '' : ' active') + '" data-genre="">Tutti</button>' +
    tags.map(function(g){
      return '<button type="button" class="luxtify-chip' + (luxtifyTag.toLowerCase() === g.toLowerCase() ? ' active' : '') + '" data-genre="' + escapeHtml(g) + '">' + escapeHtml(g) + '</button>';
    }).join('');
  Array.prototype.forEach.call(box.querySelectorAll('.luxtify-chip'), function(chip){
    chip.addEventListener('click', function(){
      luxtifyTag = chip.dataset.genre || '';
      Array.prototype.forEach.call(box.querySelectorAll('.luxtify-chip'), function(c){ c.classList.toggle('active', c === chip); }); // i filtri non si ridisegnano: la fila non torna all'inizio
      renderLuxtifyAll(true);
    });
  });
}

/* ---- album ---- */
function renderLuxtifyAlbumsRow(songs){
  var row = document.getElementById('luxtifyAlbumsRow');
  if(!row) return;
  var albums = {}, order = [];
  songs.filter(lxMatchesTag).forEach(function(s){
    var key = s.catalog_id || ('_' + s.id);
    if(!albums[key]){
      albums[key] = { catalog_id: s.catalog_id, title: (s.catalog && s.catalog.title) || s.title, cover_url: (s.catalog && s.catalog.cover_url) || s.cover_url, explicit: false, songs: [] };
      order.push(key);
    }
    if(songIsExplicit(s)) albums[key].explicit = true;
    albums[key].songs.push(s);
  });
  var heart = '<div class="lx-card" id="luxtifyLikedAlbum"><div class="lx-heart"><svg class="lx-ico" viewBox="0 0 24 24" style="width:30px;height:30px;fill:currentColor;stroke:none"><path d="M12 21s-7.5-4.6-9.5-9.2C1.2 8.6 3 5.5 6.2 5.5c1.9 0 3.2 1 3.8 2.1.6-1.1 1.9-2.1 3.8-2.1 3.2 0 5 3.1 3.7 6.3C19.5 16.4 12 21 12 21z"/></svg></div><div class="lx-card-title">Brani che mi piacciono</div></div>';
  var cards = order.map(function(k, i){
    var a = albums[k];
    var locked = a.explicit && !matureVisible;
    return '<div class="lx-card" data-album="' + i + '">' + lxCover(a.cover_url, locked) +
      '<div class="lx-card-title">' + escapeHtml(a.title) + (a.explicit ? ' <span class="lx-tag">' + (matureVisible ? 'E' : '18+') + '</span>' : '') + '</div>' +
      '<div class="lx-card-sub">Album</div></div>';
  }).join('');
  row.innerHTML = heart + cards;

  document.getElementById('luxtifyLikedAlbum').addEventListener('click', openLuxtifyLikedSongs);
  Array.prototype.forEach.call(row.querySelectorAll('[data-album]'), function(card){
    card.addEventListener('click', function(){
      var a = albums[order[Number(card.dataset.album)]];
      if(!a) return;
      if(!a.catalog_id){ openMusicPlayer(a.songs, a.title); return; } // brano senza album: parte quello
      fetchSongsForTitle(a.catalog_id).then(function(list){ openMusicPlayer(list.length ? list : a.songs, a.title); });
    });
  });
}

/* ---- classifica del sito ---- */
function renderLuxtifyTopSongs(songs){
  var section = document.getElementById('luxtifyTopSection');
  var box = document.getElementById('luxtifyTopSongs');
  if(!section || !box) return;
  var list = songs.filter(lxMatchesTag).slice().sort(function(a, b){ return (b.view_count || 0) - (a.view_count || 0); });
  if(list.length === 0){ section.classList.add('hidden'); return; }
  section.classList.remove('hidden');
  box.innerHTML = list.map(function(s, i){
    return '<div class="lx-trow" data-top-song="' + i + '"><span class="lx-num">' + (i + 1) + '</span>' + lxCover(lxSongCover(s), songIsLocked(s)) +
      '<div class="lx-trow-main"><span class="lx-trow-title">' + escapeHtml(s.title) + lxExplicitTag(s) + '</span><span class="lx-trow-sub">' + escapeHtml(s.artist || '') + '</span></div>' +
      '<button type="button" class="lx-play" aria-label="Riproduci">▶</button></div>';
  }).join('');
  Array.prototype.forEach.call(box.querySelectorAll('[data-top-song]'), function(el){
    el.addEventListener('click', function(){ openMusicPlayer(list, 'Classifica del sito', {}, Number(el.dataset.topSong)); });
  });
}

/* ridisegna le sezioni che dipendono dai brani (dopo il caricamento, dopo l'interruttore 18+, dopo un filtro) */
function renderLuxtifyAll(skipChips){
  var songs = luxtifySongsCache;
  if(!skipChips) renderLuxtifyGenreChips(songs);
  renderLuxtifyHero(songs);
  renderLuxtifyAlbumsRow(songs);
  renderLuxtifyTopSongs(songs);
}

/* pannello "In riproduzione" (schermi larghi): brano attuale e prossimi brani */
function updateLuxtifyNowPanel(title, artist, cover){
  var panel = document.getElementById('luxtifyNow');
  if(!panel) return;
  var idx = -1;
  luxtifyCurrentList.forEach(function(s, i){ if(idx === -1 && s.title === title) idx = i; });
  var next = idx === -1 ? [] : luxtifyCurrentList.slice(idx + 1, idx + 5);
  panel.innerHTML = '<h3>In riproduzione</h3>' +
    '<div class="lx-cvwrap lx-now-big"><img class="lx-cov" src="' + escapeHtml(cover || MUSIC_COVER_FALLBACK) + '" alt=""' + coverImgAttrs() + '></div>' +
    '<span class="lx-now-title">' + escapeHtml(title) + '</span><span class="lx-now-artist">' + escapeHtml(artist) + '</span>' +
    (next.length ? '<h3 style="margin-top:18px">Prossimi brani</h3>' + next.map(function(s, k){
      return '<div class="lx-trow" data-now-song="' + (idx + 1 + k) + '">' + lxCover(lxSongCover(s), songIsLocked(s)) +
        '<div class="lx-trow-main"><span class="lx-trow-title">' + escapeHtml(s.title) + lxExplicitTag(s) + '</span></div></div>';
    }).join('') : '');
  Array.prototype.forEach.call(panel.querySelectorAll('[data-now-song]'), function(el){
    el.addEventListener('click', function(){ openMusicPlayer(luxtifyCurrentList, luxtifyCurrentLabel, {}, Number(el.dataset.nowSong)); });
  });
}

function openLuxtifyLikedSongs(){
  if(!isSignedIn()){ openAuth('login'); return; }
  var uid = currentUserId();
  fetch(SUPABASE_URL + '/rest/v1/song_likes?user_id=eq.' + encodeURIComponent(uid) + '&select=songs(*,catalog(mature))', {
    headers:{ 'apikey':SUPABASE_ANON_KEY, 'Authorization':'Bearer ' + getSession().access_token }
  }).then(function(r){ return r.ok ? r.json() : []; }).then(function(rows){
    var songs = rows.map(function(r){ return r.songs; }).filter(Boolean);
    if(songs.length === 0){ window.alert(t('songs.noneYet')); return; }
    openMusicPlayer(songs, 'Brani che mi piacciono');
  });
}

function renderLuxtifyMixes(songs){
  var section = document.getElementById('luxtifyMixesSection');
  var row = document.getElementById('luxtifyMixesRow');
  ...
  fetch(SUPABASE_URL + '/rest/v1/song_likes?user_id=eq.' + encodeURIComponent(uid) + '&select=songs(*,catalog(mature))', {
    ...
    rows.forEach(function(r){
      ...
      if(!s || !s.genre) return;
      lxSongTags(s).forEach(function(tg){
        if(!byGenre[tg]) byGenre[tg] = [];
        byGenre[tg].push(s);
      });
    });
    var genres = Object.keys(byGenre).filter(function(g){ return byGenre[g].length >= 2; }); // un mix ha senso solo con almeno 2 brani dello stesso genere tra i tuoi like
    if(genres.length === 0){ section.classList.add('hidden'); return; }
    section.classList.remove('hidden');
    row.innerHTML = genres.map(function(g){
      var cover = byGenre[g][0].cover_url || '';
      return '<div class="luxtify-mix-card" data-genre="' + escapeHtml(g) + '" style="width:100px;flex-shrink:0;cursor:pointer;">' +
        '<div style="aspect-ratio:1/1;border-radius:8px;background:linear-gradient(150deg,#3a0d14,#1a0508);margin-bottom:5px;background-image:url(' + escapeHtml(cover) + ');background-size:cover;background-position:center;"></div>' +
        '<div style="color:#c9a24d;font-size:10px;font-weight:600;">Mix ' + escapeHtml(g) + '</div>' +
        '<div style="color:#8a7a5e;font-size:9px;">Basato sui tuoi like</div>' +
      '</div>';
    }).join('');
    Array.prototype.forEach.call(row.querySelectorAll('.luxtify-mix-card'), function(card){
      card.addEventListener('click', function(){
        var g = card.dataset.genre;
        openMusicPlayer(byGenre[g], 'Mix ' + g);
      });
    });
  }).catch(function(){ section.classList.add('hidden'); });
}

function renderLuxtifyTopArtists(songs){
  var section = document.getElementById('luxtifyArtistsSection');
  var row = document.getElementById('luxtifyArtistsRow');
  if(!section || !row) return;
  if(!isSignedIn()){ section.classList.add('hidden'); return; }
  var uid = currentUserId();
  fetch(SUPABASE_URL + '/rest/v1/song_likes?user_id=eq.' + encodeURIComponent(uid) + '&select=songs(artist)', {
    headers:{ 'apikey':SUPABASE_ANON_KEY, 'Authorization':'Bearer ' + getSession().access_token }
  }).then(function(r){ return r.ok ? r.json() : []; }).then(function(rows){
    var counts = {}, order = [];
    rows.forEach(function(r){
      var name = r.songs && r.songs.artist;
      if(!name) return;
      if(!counts[name]){ counts[name] = 0; order.push(name); }
      counts[name]++;
    });
    var top = order.filter(function(n){ return counts[n] >= 2; }).sort(function(a,b){ return counts[b]-counts[a]; }); // un artista "preferito" ha almeno 2 brani a cui hai messo like
    if(top.length < 2){ section.classList.add('hidden'); return; } // servono almeno 2 artisti distinti perché la sezione abbia senso
    section.classList.remove('hidden');
    row.innerHTML = top.map(function(name){
      return '<div class="luxtify-artist-card" data-artist="' + escapeHtml(name) + '" style="width:64px;flex-shrink:0;text-align:center;cursor:pointer;">' +
        '<div style="width:56px;height:56px;border-radius:50%;background:linear-gradient(150deg,#6e1423,#1a1420);margin:0 auto 5px;"></div>' +
        '<div style="color:#8a7a5e;font-size:9px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">' + escapeHtml(name) + '</div>' +
      '</div>';
    }).join('');
    Array.prototype.forEach.call(row.querySelectorAll('.luxtify-artist-card'), function(card){
      card.addEventListener('click', function(){
...
function renderLuxtifyMyPlaylists(){
  var section = document.getElementById('luxtifyMyPlaylistsSection');
  var row = document.getElementById('luxtifyMyPlaylistsRow');
  if(!section || !row) return;
  if(!isSignedIn()){ section.classList.add('hidden'); return; }
  section.classList.remove('hidden');
  loadLuxtifyMyPlaylists();

  var createBtn = document.getElementById('luxtifyCreatePlaylistBtn');
  if(createBtn){
    createBtn.onclick = function(){
      ...
      fetch(SUPABASE_URL + '/rest/v1/playlists', {
        ...
        console.warn('Creazione playlist personale fallita:', err);
        window.alert('Non è stato possibile creare la playlist.');
      });
    };
  }
}

function loadLuxtifyMyPlaylists(){
  var row = document.getElementById('luxtifyMyPlaylistsRow');
  if(!row) return;
  fetchMyPlaylists().then(function(playlists){
    var createCard = '<div class="luxtify-mix-card" id="luxtifyCreatePlaylistBtn" style="width:84px;flex-shrink:0;cursor:pointer;">' +
      '<div style="aspect-ratio:1/1;border-radius:8px;border:1px dashed rgba(201,162,77,0.4);display:flex;align-items:center;justify-content:center;color:#c9a24d;font-size:22px;margin-bottom:5px;">+</div>' +
    ...
    document.getElementById('luxtifyCreatePlaylistBtn').onclick = function(){
      var name = window.prompt('Nome della nuova playlist:');
      if(!name || !name.trim()) return;
      var session = getSession();
      fetch(SUPABASE_URL + '/rest/v1/playlists', {
...
function renderLuxtifyCollabUpload(){
  var section = document.getElementById('luxtifyCollabSection');
  if(!section) return;
  if(!isSignedIn()){ section.classList.add('hidden'); return; }
  loadOwnProfile().then(function(){
    if(!currentProfile || !currentProfile.verified){ section.classList.add('hidden'); return; }
    section.classList.remove('hidden');
    var btn = document.getElementById('btnAddLuxtifySong');
    if(btn) btn.onclick = addLuxtifySong;
  });
}

function addLuxtifySong(){
  var session = getSession();
  if(!session) return;
  var titleEl = document.getElementById('fLuxTitle');
  var artistEl = document.getElementById('fLuxArtist');
  var genreEl = document.getElementById('fLuxGenre');
  var coverFile = document.getElementById('fLuxCover').files[0];
  var audioFile = document.getElementById('fLuxAudio').files[0];
  ...
  Promise.all(uploads).then(function(urls){
    return fetch(SUPABASE_URL + '/rest/v1/songs', {
      ...
      }, (document.getElementById('fLuxExplicit') && document.getElementById('fLuxExplicit').checked) ? { explicit: true } : {})) // "explicit" si manda solo se spuntato: senza la colonna (SQL non lanciato) le canzoni normali si caricano lo stesso
    });
  }).then(function(r){
    if(!r.ok) throw new Error('salvataggio canzone fallito: ' + r.status);
    titleEl.value = ''; artistEl.value = ''; genreEl.value = '';
    document.getElementById('fLuxCover').value = '';
...
function initLuxtifyMiniBar(){
  ...
  bar.addEventListener('click', function(e){
    if(e.target.id === 'luxtifyMiniPlayPause') return; // gestito a parte, non deve riaprire il lettore
    var overlay = document.getElementById('musicPlayerOverlay');
    if(overlay){ overlay.style.display = ''; bar.classList.add('hidden'); }
  });
  document.getElementById('luxtifyMiniPlayPause').addEventListener('click', function(e){
    e.stopPropagation();
...
function ensureMusicPlayerStyle(){
  ...
  styleEl.textContent = [
    ...
    '.mp-collab b{color:var(--mp-past);font-weight:600;}',
    ...
    '.mp-eq-row{display:flex;align-items:center;gap:12px;padding:12px 2px;}',
    '.mp-eq-row label{width:52px;flex:none;font-size:13.5px;color:var(--mp-ink);}',
    '.mp-eq-row input[type="range"]{flex:1;}',
    '.mp-eq-val{width:56px;flex:none;text-align:right;font-family:"Space Mono",monospace;font-size:11px;color:var(--mp-gold);}',
...
function updateLuxtifyMiniBar(){
  var bar = document.getElementById('luxtifyMiniBar');
  if(!bar) return;
  var mediaEl = document.getElementById('musicPlayerMedia');
  if(!mediaEl){ bar.classList.add('hidden'); return; }
  ...
  playPauseBtn.textContent = mediaEl.paused ? '▶' : '⏸';
  updateLuxtifyNowPanel(titleEl ? titleEl.textContent : '', artistEl ? artistEl.textContent : '', coverEl ? coverEl.src : '');
  bar.classList.remove('hidden');
}
...
var MP_ICONS = {
  ...
  x: '<path d="M6 6l12 12M18 6L6 18"/>',
  mic: '<rect x="9" y="3" width="6" height="11" rx="3"/><path d="M6 11a6 6 0 0 0 12 0M12 17v4"/>',
  eq: '<path d="M6 20V10M6 7V4M12 20v-4M12 13V4M18 20v-7M18 10V4"/><circle cx="6" cy="8.5" r="1.6"/><circle cx="12" cy="15.5" r="1.6"/><circle cx="18" cy="11.5" r="1.6"/>'
};
...
/* ---- Equalizzatore: preferenza salvata sul dispositivo (non per brano, vale per tutti) ----
   Finché resta a 0 su tutte le bande non tocca l'audio: il brano suona esattamente come prima.
   Solo se l'ascoltatore muove almeno un cursore l'audio viene fatto passare dal grafo Web Audio. */
function mpEqDefaults(){ return { bass: 0, mid: 0, voice: 0, treble: 0 }; }
function mpGetEqGains(){
  try {
    var raw = localStorage.getItem('lux_eq');
    if(raw) return Object.assign(mpEqDefaults(), JSON.parse(raw));
  } catch(e){} // preferenza non leggibile: si riparte da zero, non è un errore che blocca l'ascolto
  return mpEqDefaults();
}
function mpSaveEqGains(g){ try { localStorage.setItem('lux_eq', JSON.stringify(g)); } catch(e){} }
function mpEqIsCustomized(g){ return g.bass !== 0 || g.mid !== 0 || g.voice !== 0 || g.treble !== 0; }
var mpAudioCtx = null; // un solo contesto audio per tutta la pagina, riusato ad ogni brano
...
function openMusicPlayer(songs, contextLabel, options, startIdx){
  ...
  if(!matureVisible && Array.isArray(songs) && songs.some(songIsExplicit)){
    var tapped = songs[startIdx || 0];
    var allowed = songs.filter(function(s){ return !songIsExplicit(s); });
    if(songIsExplicit(tapped) || allowed.length === 0){
      var originalSongs = songs;
      askLuxtifyAdult(function(){ openMusicPlayer(originalSongs, contextLabel, options, startIdx); }); // conferma dell'età, poi parte
      return;
    }
    songs = allowed; // con il 18+ spento i brani espliciti dell'elenco vengono saltati
    startIdx = allowed.indexOf(tapped);
  }
  if(Array.isArray(songs)){ luxtifyCurrentList = songs; luxtifyCurrentLabel = contextLabel || ''; }
  ...
  overlay.innerHTML =
    ...
    '<div class="mp-body" id="mpBody">' +
      ...
      '<section class="mp-tail">' +
        '<div class="mp-acts">' +
          ...
          '<button type="button" class="mp-a" id="musicPlayerEqBtn" data-mp="eqopen" aria-label="Equalizzatore">' + mpIcon('eq') + 'EQ</button>' +
        '</div>' +
    ...
    '<div class="mp-sheet" id="mpSheet"><div class="mp-sbox" id="mpSbox">' +
      ...
      '<div id="mpPanelEq" class="hidden"></div>' +
    '</div></div>' +
  ...
  var rafId = 0;
  var syncState = null;
  var eqSource = null, eqNodes = null; // ricreati ad ogni brano: un elemento audio/video si collega al grafo Web Audio una sola volta nella sua vita
  ...
  /* ---- traccia ---- */
  /* ---- Equalizzatore: si ricollega ad ogni brano, solo se l'ascoltatore lo ha già usato una volta ---- */
  function connectEq(el){
    try {
      var AC = window.AudioContext || window.webkitAudioContext;
      if(!AC) return null; // browser senza Web Audio: il brano suona comunque, solo senza equalizzatore
      if(!mpAudioCtx) mpAudioCtx = new AC();
      if(mpAudioCtx.state === 'suspended') mpAudioCtx.resume().catch(function(){});
      if(eqSource){ try { eqSource.disconnect(); } catch(e){} }
      if(eqNodes){ Object.keys(eqNodes).forEach(function(k){ try { eqNodes[k].disconnect(); } catch(e){} }); }
      var src = mpAudioCtx.createMediaElementSource(el);
      var bass = mpAudioCtx.createBiquadFilter(); bass.type = 'lowshelf'; bass.frequency.value = 200;
      var mid = mpAudioCtx.createBiquadFilter(); mid.type = 'peaking'; mid.frequency.value = 1000; mid.Q.value = 1;
      var voice = mpAudioCtx.createBiquadFilter(); voice.type = 'peaking'; voice.frequency.value = 2500; voice.Q.value = 1;
      var treble = mpAudioCtx.createBiquadFilter(); treble.type = 'highshelf'; treble.frequency.value = 6000;
      src.connect(bass); bass.connect(mid); mid.connect(voice); voice.connect(treble); treble.connect(mpAudioCtx.destination);
      eqSource = src; eqNodes = { bass: bass, mid: mid, voice: voice, treble: treble };
      applyEqGains(mpGetEqGains());
      return eqNodes;
    } catch(e){
      console.warn('Equalizzatore non disponibile su questo brano:', e);
      return null; // niente equalizzatore, ma la riproduzione normale continua
    }
  }
  function applyEqGains(g){
    if(!eqNodes) return;
    eqNodes.bass.gain.value = g.bass; eqNodes.mid.gain.value = g.mid; eqNodes.voice.gain.value = g.voice; eqNodes.treble.gain.value = g.treble;
  }
  function eqPanelHtml(){
    var g = mpGetEqGains();
    var labels = [['bass', 'Bassi'], ['mid', 'Medi'], ['voice', 'Voce'], ['treble', 'Alti']];
    return '<h3><span>Equalizzatore</span><button type="button" class="mp-ib" data-mp="sclose" style="width:32px;height:32px" aria-label="Chiudi">' + mpIcon('x', false, 18) + '</button></h3>' +
      '<div class="mp-hint">Resta salvato su questo dispositivo e si applica a ogni brano che ascolti.</div>' +
      labels.map(function(l){
        var v = g[l[0]];
        return '<div class="mp-eq-row"><label>' + l[1] + '</label><input type="range" min="-12" max="12" step="1" value="' + v + '" data-eq="' + l[0] + '" aria-label="' + l[1] + '"><span class="mp-eq-val" data-eqval="' + l[0] + '">' + (v > 0 ? '+' : '') + v + ' dB</span></div>';
      }).join('') +
      '<button type="button" class="mp-sbtn" data-mp="eqreset" style="margin-top:10px;">Azzera</button>';
  }
  function showEqPanel(){ $mp('mpPanelEq').innerHTML = eqPanelHtml(); openSheet('mpPanelEq'); }
  ...
  function playTrack(idx){
    ...
    if(song.id){
      try {
        localStorage.setItem('luxtifyLastPlayed', JSON.stringify({
          id: song.id, title: song.title, artist: song.artist || '', cover_url: song.cover_url || '',
          contextLabel: contextLabel, savedAt: Date.now()
        }));
    ...
    if(song.collaborators && song.collaborators.length){
      collabEl.innerHTML = song.collaborators.map(function(c){
        var nameLink = c.url
          ? '<a href="' + escapeHtml(c.url) + '" target="_blank" rel="noopener">' + escapeHtml(c.name) + '</a>'
          : escapeHtml(c.name);
        return (c.role ? '<b>' + escapeHtml(c.role) + ':</b> ' : '') + nameLink + verifiedBadge('verified.collaborator', !!c.verified);
      }).join(' · ');
      collabEl.classList.remove('hidden');
    } else {
      collabEl.classList.add('hidden');
    ...
    eqNodes = mpEqIsCustomized(mpGetEqGains()) ? connectEq(mediaEl) : null; // tocca il grafo audio solo se l'ascoltatore l'ha già usato
    ...
    if(song.id && social){
      ...
      mediaEl.addEventListener('play', function(){
        ...
        statsEl.textContent = (startViews + 1) + ' visualizzazioni · ' + (startPlays + 1) + ' ascolti';
      });
    }
    ...
    if('mediaSession' in navigator){
      ...
      mediaEl.addEventListener('play', function(){ navigator.mediaSession.playbackState = 'playing'; updateLuxtifyMiniBar(); });
  ...
  function openSheet(panel){
    ['mpPanelComments', 'mpPanelQueue', 'mpPanelSync', 'mpPanelEq'].forEach(function(id){ $mp(id).classList.toggle('hidden', id !== panel); });
  ...
  overlay.addEventListener('click', function(e){
    ...
    if(el){
      ...
      if(a === 'eqopen'){ if(!eqNodes) eqNodes = connectEq(mediaEl); showEqPanel(); return; }
      if(a === 'eqreset'){ var g0 = mpEqDefaults(); mpSaveEqGains(g0); applyEqGains(g0); showEqPanel(); return; }
  ...
  overlay.addEventListener('input', function(e){
    ...
    if(e.target.dataset && e.target.dataset.eq){
      var key = e.target.dataset.eq, val = parseInt(e.target.value, 10);
      var gains = mpGetEqGains(); gains[key] = val; mpSaveEqGains(gains);
      if(!eqNodes) eqNodes = connectEq(mediaEl); // prima modifica in assoluto: collega ora, senza aspettare il brano dopo
      else applyEqGains(gains);
      var valEl = overlay.querySelector('[data-eqval="' + key + '"]');
...
function loadSongComments(songId, listEl, countEl){
  if(!listEl) return;
  ...
  fetch(SUPABASE_URL + '/rest/v1/comments?song_id=eq.' + encodeURIComponent(songId) + '&select=*&order=created_at.desc', { headers: headers })
    ...
    .then(function(rows){
      if(countEl) countEl.querySelector('span').textContent = rows.length;
      listEl.innerHTML = '';
      if(rows.length === 0){ listEl.innerHTML = '<p class="form-note">' + t('comments.empty') + '</p>'; return; }

      // lookup separato dei profili per sapere chi ha la spunta verificata,
...
function submitSongComment(songId, inputEl, listEl, errEl, countEl){
  ...
  nameStep.then(function(profile){
    ...
    loadSongComments(songId, listEl, countEl);
  }).catch(function(e){
...
function destroyMusicPlayerOverlay(){
  var overlay = document.getElementById('musicPlayerOverlay');
  if(overlay) overlay.remove();
  if('mediaSession' in navigator){
  ...
  var miniBar = document.getElementById('luxtifyMiniBar');
...
function closeMusicPlayer(){
  ...
  // Su Luxtify (dove esiste una mini-barra), chiudere il lettore a schermo
  // intero MINIMIZZA soltanto — la musica continua, come su Spotify.
  // Sulle altre pagine del sito (lettura fumetti) il comportamento resta
  // quello di sempre: chiudere ferma tutto.
  var miniBar = document.getElementById('luxtifyMiniBar');
  var mediaEl = document.getElementById('musicPlayerMedia');
  if(miniBar && mediaEl){
    overlay.style.display = 'none';
    updateLuxtifyMiniBar();
    return;
  }
...
function addSong(){
  ...
  var genreEl = document.getElementById('fSongGenre');
  ...
  Promise.all(uploads).then(function(urls){
    ...
    if(!r.ok) throw new Error('salvataggio canzone fallito: ' + r.status);
    titleEl.value = ''; artistEl.value = ''; genreEl.value = ''; lyricsEl.value = '';
...
function collectSongCollabRows(){
  ...
  Array.prototype.forEach.call(rows, function(row){
    ...
    if(!name) return;
    var roleEl = row.querySelector('.fSongCollabRole');
    out.push({
      name: name,
      role: roleEl ? (roleEl.value.trim() || null) : null,
...
function songCollabRowHtml(){
  return '<div class="song-collab-row" style="display:flex;gap:6px;margin-bottom:4px;align-items:center;flex-wrap:wrap;">' +
    '<input type="text" class="fSongCollabName" placeholder="Nome" style="flex:1 1 110px;">' +
    '<input type="text" class="fSongCollabRole" placeholder="Ruolo (es. Voce, Testo)" style="flex:1 1 110px;">' +
    '<input type="url" class="fSongCollabUrl" placeholder="Link Instagram (opz.)" style="flex:1 1 140px;">' +
...
function resetSongCollabRows(){
  ...
  if(!wrap) return;
  wrap.innerHTML = songCollabRowHtml();
}
function addSongCollabRow(){
  ...
  var row = document.createElement('div');
  row.innerHTML = songCollabRowHtml();
...
function loadEventsAdminList(){
  ...
  if(!list) return;
  fetch(SUPABASE_URL + '/rest/v1/community_events?select=*&order=event_date.desc', { headers: communityHeaders() })
    .then(function(r){ return r.ok ? r.json() : []; })
    .then(function(events){
    ...
    .catch(function(e){
      list.innerHTML = '<p class="form-note">Non riesco a caricare gli eventi al momento.</p>';
...
function addEvent(){
  var session = getSession();
  if(!session) return;
  var titleEl = document.getElementById('fEventTitle');
  ...
  if(imgFile){
    ...
    imgPromise = fetch(SUPABASE_URL + '/storage/v1/object/events/' + imgPath, {
      method:'POST',
      headers:{ 'apikey':SUPABASE_ANON_KEY, 'Authorization':'Bearer ' + session.access_token, 'Content-Type': imgFile.type || 'image/jpeg' },
      body: imgFile
    }).then(function(r){
      if(!r.ok) throw new Error('upload immagine evento fallito: ' + r.status);
      return SUPABASE_URL + '/storage/v1/object/public/events/' + imgPath;
    });
  }
...
function loadServersAdminList(){
  var list = document.getElementById('adminServersList');
  if(!list) return;
  fetch(SUPABASE_URL + '/rest/v1/communities?select=*&order=created_at.desc', { headers: communityHeaders() })
    .then(function(r){ return r.ok ? r.json() : []; })
    .then(function(servers){
      if(servers.length === 0){ list.innerHTML = '<p class="form-note">Nessun server ancora.</p>'; return; }
      return Promise.all(servers.map(function(s){
        return fetch(SUPABASE_URL + '/rest/v1/channels?community_id=eq.' + encodeURIComponent(s.id) + '&select=id', { headers: communityHeaders() })
          .then(function(r){ return r.ok ? r.json() : []; })
          .then(function(chans){ s._channelCount = chans.length; return s; });
      })).then(function(servers){
        list.innerHTML = servers.map(function(s){
          return '<div class="admin-list-row" style="border-bottom:1px solid var(--line);padding:10px 0;display:flex;align-items:center;gap:10px;">' +
            ...
            '<div style="flex:1;">' +
              ...
              '<div style="font-size:11px;color:var(--parchment-dim);">' + s._channelCount + ' canal' + (s._channelCount === 1 ? 'e' : 'i') + '</div>' +
            '</div>' +
            '<button type="button" class="btn btn-sm btn-ghost" data-del-server="' + s.id + '" style="border-color:#e24b4a;color:#e24b4a;">Elimina</button>' +
          '</div>';
        }).join('');
        Array.prototype.forEach.call(list.querySelectorAll('[data-del-server]'), function(btn){
          btn.addEventListener('click', function(){ deleteServer(btn.dataset.delServer); });
        });
      });
    })
    .catch(function(e){
      list.innerHTML = '<p class="form-note">Non riesco a caricare i server al momento.</p>';
...
function addServer(){
  var session = getSession();
  if(!session) return;
  var nameEl = document.getElementById('fServerName');
  var iconEl = document.getElementById('fServerIcon');
  var descEl = document.getElementById('fServerDesc');
  var errEl = document.getElementById('serverError');
  errEl.textContent = '';
  if(!nameEl.value.trim()){ errEl.textContent = 'Scrivi un nome.'; return; }

  fetch(SUPABASE_URL + '/rest/v1/communities', {
    method:'POST',
    headers:{ 'apikey':SUPABASE_ANON_KEY, 'Authorization':'Bearer ' + session.access_token, 'Content-Type':'application/json' },
    body: JSON.stringify({
    ...
    if(!r.ok) throw new Error('creazione server fallita: ' + r.status);
    nameEl.value = ''; iconEl.value = '💬'; descEl.value = '';
    loadServersAdminList();
  }).catch(function(err){
    console.warn('Creazione server fallita:', err);
    errEl.textContent = 'Errore: ' + err.message;
  });
}
...
function addPlaylist(){
  ...
  coverPromise.then(function(coverUrl){
    return fetch(SUPABASE_URL + '/rest/v1/playlists', {
      method:'POST',
      headers:{ 'apikey':SUPABASE_ANON_KEY, 'Authorization':'Bearer ' + session.access_token, 'Content-Type':'application/json' },
      body: JSON.stringify({ title: titleEl.value.trim(), description: descEl.value.trim() || null, cover_url: coverUrl, created_by: currentUserId(), is_public: true })
    });
  }).then(function(r){
    if(!r.ok) throw new Error('creazione playlist fallita: ' + r.status);
    ...
    if(manualBtn){
      ...
      if(wifi !== true){
        if(!hintEl){
          ...
          hintEl.className = 'mycomics-hint-note';
      ...
      if(!expiryEl){
        ...
        expiryEl.className = 'mycomics-hint-note';
...
var characterImages = {};
var CHAR_LIST = ['Lucifer','Lilith','Lucifera','Lucio','Nox'];
...
function __appInit(){
  ...
  document.getElementById('btnMainMenu') && document.getElementById('btnMainMenu').addEventListener('click', function(e){
    e.stopPropagation();
    var trig = document.getElementById('btnMainMenu');
    var nav = document.getElementById('mainNavDropdown');
  ...
  document.addEventListener('click', function(e){
    var trig = document.getElementById('btnMainMenu');
    var nav = document.getElementById('mainNavDropdown');
    if(!trig || !nav || !nav.classList.contains('open')) return;
    if(!nav.contains(e.target) && e.target !== trig && !trig.contains(e.target)){
  ...
  document.getElementById('serverTeaser') && document.getElementById('serverTeaser').addEventListener('click', function(){ switchCommunityTab('servers'); });
  document.querySelectorAll('.diary-kind-tab').forEach(function(btn){
