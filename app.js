...
function loadUserDirectory(){
  ...
  fetch(SUPABASE_URL + '/rest/v1/profiles?select=id,display_name,last_active_at&order=display_name.asc', { headers: communityHeaders() })
    ...
    .then(function(rows){
      ...
      rows.forEach(function(p){ p._online = isOnlineSince(p.last_active_at); });
      ...
      rows.forEach(function(p){
        ...
        row.innerHTML =
          ...
          '<span class="user-directory-status">'+(p._online ? t('userDir.online') : t('userDir.offline'))+'</span>';
