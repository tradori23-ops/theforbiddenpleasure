...
function loadUserDirectory(){
  ...
  var profilesStep = fetch(SUPABASE_URL + '/rest/v1/profiles?select=id,display_name,last_active_at,verified&order=display_name.asc', { headers: communityHeaders() })
    .then(function(r){ if(!r.ok) throw new Error('user directory read failed'); return r.json(); });
  ...
  Promise.all([profilesStep, friendsStep]).then(function(results){
    ...
    function renderRow(p, showLastSeen){
      var row = document.createElement('button');
      row.type = 'button';
      row.className = 'user-directory-row';
      var dotClass = p._online ? 'user-directory-dot online' : 'user-directory-dot offline';
      var rightHtml = showLastSeen
        ? '<span class="user-directory-status">'+(p._online ? t('userDir.online') : (p.last_active_at ? notifTimeAgo(p.last_active_at) : t('userDir.offline')))+'</span>'
        : '<i class="user-directory-verified-check" aria-hidden="true">✓</i>';
      row.innerHTML =
        '<span class="'+dotClass+'"></span>'+
        '<span class="user-directory-name">'+escapeHtml(p.display_name || t('notif.someone'))+'</span>'+
        rightHtml;
    ...
    if(verifiedFriends.length > 0){
      ...
      verifiedFriends.forEach(function(p){ list.appendChild(renderRow(p, false)); });
    }
    if(others.length > 0){
      ...
      others.forEach(function(p){ list.appendChild(renderRow(p, true)); });
    }
  }).catch(function(e){
    list.innerHTML = '';
